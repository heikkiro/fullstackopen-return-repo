import { useState } from 'react'

import './App.css'

// Component to display anectode with random or defined index
const DisplayAnecdote = (props) => {
  return (
    <div>
      "{props.anecdote[props.index]}"
    </div>
  )
}

// Button component
const Button = (props) => (
  <button onClick={props.onClick}>
    {props.text}
  </button>
)


const App = () => {
  const anecdotes = [
    'If it hurts, do it more often.',
    'Adding manpower to a late software project makes it later!',
    'The first 90 percent of the code accounts for the first 90 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
    'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
    'Premature optimization is the root of all evil.',
    'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
    'Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when dianosing patients.',
    'The only way to go fast, is to go well.'
  ]

  // Initialize array for votes state hook
  const initialVotes = Array(anecdotes.length).fill(0);

  const [selected, setSelected] = useState(0)
  const [votes, setVotes] = useState(initialVotes);

  // Event handler for selecting random anecdote 
  const handleSelected = () => {
    setSelected(Math.floor(Math.random() * (anecdotes.length - 1)))
  }

  // Event handler to update votes array
  const handleVote = () => {
    const nextVotes = [...votes]
    nextVotes[selected] += 1
    setVotes(nextVotes);
  }

  // Function to return the anecdote with most votes
  function findMostVoted() {
    let maxVotes = Math.max(...votes)
    return votes.indexOf(maxVotes)
  }

  return (
    <div>
      <h1>Software Engineering Anecdotes</h1>
      <h2>Anecdote of the day</h2>
      <DisplayAnecdote anecdote={anecdotes} index={selected} />
      <div>
        <div>
          has {votes[selected]} votes
        </div> <br></br>
        <Button onClick={handleVote} text="vote" />
        <Button onClick={handleSelected} text="next anecdote" /> 
        <div> 
          <h2>Anecdote with most votes</h2>
          <DisplayAnecdote anecdote={anecdotes} index={findMostVoted()} />
        </div>
      </div>
    </div>
  )
}


export default App